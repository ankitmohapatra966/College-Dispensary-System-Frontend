...
const Login = (props) => {
  const navigate=useNavigate();
    ...
    const handleLogin= async()=>{
        ...
        await axios.post(`${BASE_URL}/api/auth/login`,loginField,{withCredentials:true}).then((response)=>{
    ...
    const handleRegister = async () =>{
      if(registerField.email.trim() === "" || registerField.password.trim()==="" || registerField.name.trim()==="" || registerField.roll.trim() === "" ) return toast.error("Please Enter all the credential");
      if(registerField.name.length<3) return toast.error("Name should be greater than 2 characters")
        props.showLoader();
      await axios.post(`${BASE_URL}/api/auth/register`,registerField).then(response =>{
    ...
    <div className='login-page'>
      ...
      <div className="signup-page-card">
        ...
        <div className="form-input-fields">
          ...
          <input value={registerField.email} onChange={(event)=>handleOnChange(event,'email','register')} className="form-input" type='email' placeholder='Enter Your Email Id' />
