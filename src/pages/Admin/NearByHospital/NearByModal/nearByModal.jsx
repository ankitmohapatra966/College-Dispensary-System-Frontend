...
useEffect(() => {
    ...
    const updateFunc = async () => {
        await axios.put(`${import.meta.env.VITE_API_BASE_URL}/api/hospital/update/${clickedItem._id}`,inputField,{withCredentials:true}).then((response)=>{
    ...
    const handleSubmit = async(e) =>{
        e.preventDefault();
         if (inputField.name.trim().length === 0 || inputField.address.trim().length === 0 || inputField.contact.trim().length === 0) return toast.error("Enter all the fields")
            ...
            await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/hospital/add`,inputField,{withCredentials:true}).then((response)=>{
                window.location.reload();
        }).catch((err) => {
                toast.error(err?.response?.data?.error);
              })
